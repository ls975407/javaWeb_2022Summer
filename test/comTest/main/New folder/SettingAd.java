...
public class SettingAd implements Setting {
	private final String INI_SETTING_FILE;
	...
	void initialize(String dirWork, Strin dirStorage, String dirUser) {
		_dirWork = dirWork; _dirStorage = dirStorage; _dirUser = dirUser;
	}
	
	private SettingAd(String workDir, String filename) {
		INI_SETTING_FILE = filename;
		INI_SETTING_DIR = workDir;
		INI_SETTING_FILE_PATH = workDir + "/" + filename;
		
	}
	private static SettingAd _ptr = null;
	static public SettingAd ptr() throws JSONException {
		if ( _ptr == null) {
			_ptr = new SettingAd();
			_ptr.parseJson(FileM.readJsonObject(INI_SETTING_FILE));
		}
		return _ptr;
	}
	static public FrameM readIniFrame() throws JSONException {
		JSONObject json = FileM.readJsonObject(INI_FRAME_FILE);
		return findFrameObj(json.getString("FrameName"));
	}
	static public writeIniFrame(FrameM frameM) throws IOException {
		JSONObject json = new JSONObject();
		json.add("FrameName", frameM._keyName);
		FileM.write(INI_FRAME_FILE, json.toString());
	}
	public String readCloudNode() throws JSONException { 
		JSONObject json = FileM.readJsonObject(INI_CLOUD_NODE);
		return findFrameObj(json.getString("CloudNode"));
	}
	...
	private void parseJson(JSONObject json) throws JSONException {
		...
		_tFrameObj = _iniTS2F(json.getJSONArray("TFrameObj"), new FrameIniImp(this));
	}
	
	private static class FrameIniImp implements FrameIni {
		private final int _screenX, _screenY;
		private final String _pathLFImage;
		private final String[] _aLegalImageExtension;
		FrameIniImp(SettingAd constSet) {
			_screenX = constSet._screenX;
			_screenY = constSet._screenY;
			_pathLFImage = constSet._pathLFImage;
			_aLegalImageExtension = constSet._aLegalImageExtension;
		}
		int getScreenX() { return _screenX; }
		int getScreenY() { return _screenY; }
		String getPathLFImage() { return _pathLFImage; }
		String[] getLegalImageExtension() { return _aLegalImageExtension; }
	}
	...
	static private Map<String, FrameM> _iniTS2F(JSONArray jsonA, FrameIni frameIni) throws JSONException {
