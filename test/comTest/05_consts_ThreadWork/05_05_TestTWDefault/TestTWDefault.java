/*
javac -encoding utf8  TestTWDefault.java
*/
...
public class TestTWDefault {	
	...
	public static void main(String[] arg) {
		...
		println(ThreadWork.getDefaultKCh(PathGet.getLocal(), kid)._keyName);
		println(ThreadWork.getDefaultKCh(PathGet.getLocal(), kid, new InforCh(kchs[0])._mark)._keyName);
		println(ThreadWork.getCasheKPgS(PathGet.getLocal(), kid, new InforCh(kchs[0])._mark)._keyName);
		println(ThreadWork.getCasheKPgS(PathGet.getLocal(), kchs[0])._keyName);
		for (TokenU tokenU: ThreadWork.getKPgSF(PathGet.getLocal(), new KChT(kchs[0]))) {
